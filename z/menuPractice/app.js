const menu = [
  ...
  function displayMenuItems(menuItem){
    let catagories= menuItem.map(item => {
    ...
    section.innerHTML=catagories
  };

 function displayMenuBtn(){
  let catagories=menu.reduce((value,item)=>{
  ...
  console.log(catagories);
  let catagorybtns=catagories.map((item)=>{
   return `<button type="button" class="filter-btn" data-id=${item}>
          ${item}
        </button>`
  });
  catagorybtns=catagorybtns.join("");
  btn.innerHTML=catagorybtns
  ...
  filterBtn.forEach((btn)=>{
    btn.addEventListener("click",(e)=>{
        ...
        let menuCatagory= menu.filter((item)=>{
